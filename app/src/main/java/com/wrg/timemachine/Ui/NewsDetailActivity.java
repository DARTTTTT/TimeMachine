...
import static com.wrg.timemachine.R.id.webview;

/**
 * Created by 翁 on 2017/3/8.
 */
@SuppressLint("SetJavaScriptEnabled")
public class NewsDetailActivity extends BaseActivity implements MyRetrofitUtil.DownListener {
    ...
    @OnClick({R.id.back, R.id.next, long_comments})
 ...
 /* scrollView.setScrollViewListener(new MyScrollView.ScrollViewListener() {
            ...
            public void onScrollChanged(View scrollView, int x, int y, int oldx, int oldy) {
                Log.d("print", "onScrollChanged:x-----------> " + x + "y---------->" + y + "oldx----------->" + oldx + "oldy------------->" + oldy);
                //第一种
                if (y < 500) {
                    float alpha = 1 - ((float) y) / 500;
                    toolbar.setAlpha(alpha);
                    Log.d("print", "onScrollChanged: 透明度的数值" + alpha);
                    if (alpha == 0) {
